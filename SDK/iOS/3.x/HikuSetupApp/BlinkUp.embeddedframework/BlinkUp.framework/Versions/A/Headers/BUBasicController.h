 ...
 */
typedef void (^BUResignActiveBlock)(BOOL willRespond, BOOL userDidCancel, NSError *error);
 ...
 */
typedef void (^BUDevicePollingDidCompleteBlock)(BUDeviceInfo *deviceInfo, BOOL timedOut, NSError *error);
 ...
 *  @return An initialized BlinkUpController
 */
- (instancetype)initWithApiKey:(NSString *)apiKey planId:(NSString *)planId;
...
- (void)presentInterfaceAnimated:(BOOL)animated
  resignActive:(BUResignActiveBlock)resignActive
  devicePollingDidComplete:(BUDevicePollingDidCompleteBlock)devicePollingDidComplete;
 ...
 */
- (void)forceDismissWithCompletionHandler:(void (^)())completionHandler;

/*!
 *  @brief  An interface for network selection by the user
 *
 */
@property (strong, nonatomic, readonly) BUNetworkSelectController *networkSelectController;

/*!
 *  @brief  An interface that performs the BlinkUp flashing operations
 */
@property (strong, nonatomic, readonly) BUFlashController *flashController;

/*!
 *  @brief  Gather device metadata after a flash
 */
@property (strong, nonatomic, readonly) BUDevicePoller *devicePoller;
